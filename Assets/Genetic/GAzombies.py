...
class Individual_Grid(object):
    ...
    def mutate(self, genome):
        hWallOffsetX = 15 #how much space between horizontal walls that are needed

        vWallOffsetX = 10 #how much space between vertical walls that are needed

        new_genome = copy.deepcopy(self.genome) #make a copy of the genome were working with
        
        for y in range(height):
            itemsRemoved = 100 #maximum items that can be removed per row
            
            itemsAdded = 5 #maximum items that can be added per row, resets to 3 every row
            ...
            for x in range(width):
                if new_genome[y][x] != '-' and itemsRemoved > 0 and random.randint(1, 30) == 10: #if (y, x) on genome is not empty i.e its a prefab
                    ...
                    if x >= addLimit and itemsAdded > 0 and y % 2 == 0 and random.randint(1, 100) == 10: #if (y, x) is empty ('-') and y is even -> place prefab
                        ...
                        if prefab == 'N':
                            #vertical variant
                            addLimit = x + vWallOffsetX
                            new_genome[y][x] = prefab
                        if prefab == "B": 
                            #horizontal variant
                            addLimit = x + hWallOffsetX
                            new_genome[y][x] = prefab
                            
                        #add another if here and in create rooms    
    ...
    def jrod_individual(cls):
        ...
        rooms = 6 #number of rooms to generate
        ...
        for y in range(height):
            ...
            for x in range(width):
                ...
                if random.randint(1, 30) == 10 and rocksR > 0 and y % 2 == 0: #y must be an even number for rock to be placed on (y, x), done to not place rocks on every row
                    g[y][x] = "R"
                    rocksR = rocksR - 1
                if random.randint(1, 30) == 10 and rocksN > 0 and y % 2 != 0: #y must be an even number for rock to be placed on (y, x), done to not place rocks on every row
                    g[y][x] = "M"
                    rocksN = rocksN - 1
        return cls(g)
    ...
    def random_individual(cls):
        ...
        rooms = 9 #number of rooms to generate
        ...
        for y in range(height):
            ...
            for x in range(width):
                #random.randint(a, b) = c determines % for a rock to be placed at (y, x) , 2 rocks total can be placed per line
                if random.randint(1, 25) == 10 and rocksR > 0 and y % 2 != 0: #y must be an even number for rock to be placed on (y, x), done to not place rocks on every row
                    g[y][x] = "R"
                    rocksR = rocksR - 1
                if random.randint(1, 40) == 10 and rocksN > 0 and y % 2 == 0: #y must be an even number for rock to be placed on (y, x), done to not place rocks on every row
                    g[y][x] = "M"
                    rocksN = rocksN - 1
        return cls(g)
...
def genomeClear(genome):
    ...
    for y in range(height):
        for x in range(width):
            if random.randint(1, 15) == 1 and y % 2 == 0 and genome[y][x] != "S": #make it a low chance of replacing (y, x) with "-"
                genome[y][x] = "-"
            elif genome[y][x] == "S":
                spawnCounter = spawnCounter + 1 
    
    if spawnCounter < spawners:
        for y in range(height):
            for x in range(width):
...
def deClutter(genome):
    spawners = spawnPoints
    spawnCounter = 0
    for y in range(height):
        for x in range(width):
            if random.randint(1, 10) == 1 and genome[y][x] != "S": #make it a low chance of replacing (y, x) with "-"
                genome[y][x] = "-"                   
    return genome
...
def adj(all_coordinates, current_node, genome):
    ...
    try:
        toprightCell = (current_node[0] - 1, current_node[1] + 1)
        index = all_coordinates.index(toprightCell)
        eDistance(genome, toprightCell, current_node, adjList)
    except ValueError:
        #cell doesn't exist don't add to list
        pass
        
    try:
        topleftCell = (current_node[0] - 1, current_node[1] - 1)
        index = all_coordinates.index(topleftCell)
        eDistance(genome, topleftCell, current_node, adjList)
    except ValueError:
        #cell doesn't exist don't add to list
        pass
    
    try:
        bottomleftCell = (current_node[0] + 1, current_node[1] - 1)
        index = all_coordinates.index(bottomleftCell)
        eDistance(genome, bottomleftCell, current_node, adjList)
    except ValueError:
        #cell doesn't exist don't add to list
        pass
    
    try:
        bottomrightCell = (current_node[0] + 1, current_node[1] + 1)
        index = all_coordinates.index(bottomrightCell)
        eDistance(genome, bottomrightCell, current_node, adjList)
    except ValueError:
        #cell doesn't exist don't add to list
        pass
...
def clearDijkstraPath(genome, cell):
    try:
        #index = genome.index((cell[0] - 1, cell[1] + 1))
        if cell[0] - 1 <= height - 1 and cell[1] + 1 <= width - 1 and genome[cell[0] - 1][cell[1] + 1] != "S" and genome[cell[0] - 1][cell[1] + 1] != "O" and genome[cell[0] - 1][cell[1] + 1] != "X":
           genome[cell[0] - 1][cell[1] + 1] = "X"
    except ValueError:
        pass
    ...
    try:
        #index = genome.index((cell[0] - 1, cell[1]))
        if cell[0] - 1 <= height - 1 and cell[1] <= width - 1 and genome[cell[0] - 1][cell[1]] != "S" and genome[cell[0] - 1][cell[1]] != "O" and genome[cell[0] - 1][cell[1]] != "X":
           genome[cell[0] - 1][cell[1]] = "X"
    except ValueError:
        pass
    ...
    try:
        #index = genome.index((cell[0], cell[1] - 1))
        if cell[0] <= height - 1 and cell[1] - 1 <= width - 1 and genome[cell[0]][cell[1] - 1] != "S" and genome[cell[0]][cell[1] - 1] != "O" and genome[cell[0]][cell[1] - 1] != "X":
           genome[cell[0]][cell[1] - 1] = "X"
    except ValueError:
        pass
        ...
        try:
            while True:
                ...
                if generation > 0:
                    ...
                    deClutterRange = 5
                    if generation > 5:
                        for i in range(deClutterRange):
                            deClutter(bestGenome)
                    else:
                        deClutter(bestGenome)
                    ...
                    for y in range(-9, 10):
                        for x in range(-7, 8):
                            if y == 0 and x == 0:
                                ...
                                if random1[0] + y <= height - 1 and random1[1] + x <= width - 1:
                                    bestGenome[random1[0] + y][random1[1] + x] = "O"
                                if random2[0] + y <= height - 1 and random2[1] + x <= width - 1:
                    ...
                    #bestGenome = clearDijkstraPath(bestGenome, allPaths)
                    ...
                    for paths in allPaths: #allPaths = [path1, path2, path3] where path1 = [(y, x), (y, x)]
                        for coordinates in paths: #coordinates = (y, x)
                            try:
                                ...
                                clearDijkstraPath(bestGenome, coordinates)   
